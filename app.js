...
canvas.addEventListener('mousedown', (e) => {
  ...
  // التحقق من النصوص بناءً على الإحداثيات الظاهرة
  for (const key in textPositions) {
    ...
    if (isMouseOverText(mouseX, mouseY, text, textPos.x, textPos.y)) {
      isDragging = true;
      draggedText = key;
      offsetX = mouseX - textPos.x;
      offsetY = mouseY - textPos.y;
      break;
    }
  }
});
...
canvas.addEventListener('mousemove', (e) => {
  if (isDragging && draggedText) {
    ...
    textPositions[draggedText].x = mouseX - offsetX;
    textPositions[draggedText].y = mouseY - offsetY;
